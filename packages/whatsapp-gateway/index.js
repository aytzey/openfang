...
const PULSIVO_SALESMAN_URL = (process.env.PULSIVO_SALESMAN_URL || 'http://127.0.0.1:4200').replace(/\/+$/, '');
const DEFAULT_AGENT = process.env.PULSIVO_SALESMAN_DEFAULT_AGENT || 'assistant';
...
async function startConnection() {
  ...
  // Incoming messages → forward to PulsivoSalesman
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Forward to PulsivoSalesman agent
      try {
        const response = await forwardToPulsivoSalesman(text, phone, pushName);
        if (response && sock) {
...
// ---------------------------------------------------------------------------
// Forward incoming message to PulsivoSalesman API, return agent response
// ---------------------------------------------------------------------------
function forwardToPulsivoSalesman(text, phone, pushName) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${PULSIVO_SALESMAN_URL}/api/agents/${encodeURIComponent(DEFAULT_AGENT)}/message`);
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('PulsivoSalesman API timeout'));
    });
    req.write(payload);
    req.end();
  });
...
// ---------------------------------------------------------------------------
// Send a message via Baileys (called by PulsivoSalesman for outgoing)
// ---------------------------------------------------------------------------
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[gateway] PulsivoSalesman URL: ${PULSIVO_SALESMAN_URL}`);
