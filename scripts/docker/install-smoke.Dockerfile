...
#   docker build -f scripts/docker/install-smoke.Dockerfile \
#     --build-arg PULSIVO_SALESMAN_SMOKE_FULL=1 .
...
ARG PULSIVO_SALESMAN_SMOKE_FULL=0
RUN if [ "$PULSIVO_SALESMAN_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$PULSIVO_SALESMAN_SMOKE_FULL" = "1" ] && [ -f "$HOME/.pulsivo-salesman/bin/pulsivo-salesman" ]; then \
        $HOME/.pulsivo-salesman/bin/pulsivo-salesman --version && \
        echo "PASS: pulsivo-salesman binary works"; \
    else \
