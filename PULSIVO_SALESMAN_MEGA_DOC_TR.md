# Pulsivo Salesman Signal-to-Meeting Engine — Birlesmis Mega Dokuman
...
**PARCA 1** — [Pulsivo Salesman Platform Genel Bakisi](#parca-1--pulsivo-salesman-platform-genel-bakisi)
...
===============================================================
# PARCA 1 — PULSIVO_SALESMAN PLATFORM GENEL BAKISI
===============================================================

# Pulsivo Salesman Sistem Analizi - Kapsamli Teknik Dokumantasyon

> Bu dokuman, Pulsivo Salesman Agent Isletim Sistemi'nin tum alt sistemlerini, is akislarini,
...
**Pulsivo Salesman**, Rust dilinde yazilmis acik kaynakli bir **Agent Isletim Sistemi**dir.
...
Kullanici (CLI / Desktop / SDK / Web Dashboard)
   ...
   HTTP/WS API Sunucusu (port 4200)
         |
         v
   Pulsivo Salesman Kernel (Agent Isletim Sistemi Cekirdegi)
         |
    +----+----+----+----+
    |    |    |    |    |
    v    v    v    v    v
...
```
pulsivo-salesman-types           (sifir bagimllik, temel tipler)
    |
    +-- pulsivo-salesman-memory      (SQLite + semantik arama + bilgi grafi)
    +-- pulsivo-salesman-wire        (OFP ag protokolu, TCP + HMAC)
    +-- pulsivo-salesman-channels    (40 mesajlasma adaptoru)
    +-- pulsivo-salesman-skills      (beceri kayit + yukleme + pazar yeri)
    +-- pulsivo-salesman-hands       (otonom paket agentlar)
    +-- pulsivo-salesman-extensions  (MCP sunucu + kimlik kasasi + OAuth)
    +-- pulsivo-salesman-migrate     (OpenClaw/LangChain goc araci)
    |
    +-- pulsivo-salesman-runtime     (LLM suruculeri, arac calistirici, sandbox)
    |       |
    |       +-- pulsivo-salesman-kernel  (yasam dongusu, zamanlayici, is akisi, olay bus)
    |               |
...
|-------|-----------|-------------|
| **pulsivo-salesman-types** | `crates/pulsivo-salesman-types/` | Paylasilan veri yapilari (agent, mesaj, arac, olay, yapilandirma) |
| **pulsivo-salesman-memory** | `crates/pulsivo-salesman-memory/` | Birlesik bellek API: SQLite + semantik arama + bilgi grafi + oturum |
| **pulsivo-salesman-runtime** | `crates/pulsivo-salesman-runtime/` | LLM suruculeri (20+), arac calistirma, WASM sandbox, tarayici kontrolu |
...
| **pulsivo-salesman-extensions** | `crates/pulsivo-salesman-extensions/` | Entegrasyon pazari + AES-256-GCM kimlik kasasi + OAuth2 PKCE |
| **pulsivo-salesman-migrate** | `crates/pulsivo-salesman-migrate/` | OpenClaw ve diger frameworklerden goc araci |
| **pulsivo-salesman-desktop** | `crates/pulsivo-salesman-desktop/` | Tauri 2.0 masaustu uygulamasi (macOS/Windows/Linux) |
...
**Dosya:** `crates/pulsivo-salesman-kernel/src/kernel.rs` (~5000 satir)
...
**Dosya:** `crates/pulsivo-salesman-runtime/src/agent_loop.rs` (~1000+ satir)
...
**Dosya:** `crates/pulsivo-salesman-runtime/src/llm_driver.rs` + `drivers/`
...
**Dosya:** `crates/pulsivo-salesman-runtime/src/tool_runner.rs` (~1500 satir)
...
**Dosya:** `crates/pulsivo-salesman-memory/src/` (birden fazla dosya)
...
**Dosya:** `crates/pulsivo-salesman-api/src/sales.rs` (~14.148 satir)

Pulsivo Salesman'in en kapsamli alt sistemlerinden biri. Tam otomatik B2B satis motoru.
...
**Dosyalar:**
- `crates/pulsivo-salesman-api/static/index_body.html` (~27KB)
- `crates/pulsivo-salesman-api/static/js/pages/sales.js` (~611 satir)
- `crates/pulsivo-salesman-api/static/js/api.js` (~310 satir)
- `crates/pulsivo-salesman-api/static/css/` (tema, duzen, bilesenler)
...
**Dosya:** `crates/pulsivo-salesman-api/src/codex_oauth.rs` (~1335 satir)
...
### 15.1 OFP (Pulsivo Salesman Protocol) - Mesh Aglama

**Dosya:** `crates/pulsivo-salesman-wire/src/`
...
**Dosya:** `crates/pulsivo-salesman-runtime/src/a2a.rs`
...
```
Pulsivo Salesman Kernel A              Pulsivo Salesman Kernel B
...
**Dosya:** `crates/pulsivo-salesman-kernel/src/workflow.rs` (~1367 satir)
...
**Dosya:** `crates/pulsivo-salesman-kernel/src/cron.rs` (~693 satir)
...
- Gorevler JSON olarak `~/.pulsivo-salesman/data/cron_jobs.json` dosyasina kaydedilir
...
**Dosya:** `crates/pulsivo-salesman-kernel/src/background.rs` (~457 satir)
...
**Dosya:** `crates/pulsivo-salesman-channels/src/`
...
**Dosya:** `crates/pulsivo-salesman-types/src/config.rs` (~3554 satir)

### Ana Yapilandirma Dosyasi: `~/.pulsivo-salesman/config.toml`
...
|-------|-----------|-------|
| `agents` | pulsivo-salesman.db | Kalici agent kayitlari |
| `sessions` | pulsivo-salesman.db | Konusma gecmisleri |
| `memories` | pulsivo-salesman.db | Semantik bellek kayitlari |
| `entities` | pulsivo-salesman.db | Bilgi grafi varliklari |
| `relations` | pulsivo-salesman.db | Bilgi grafi iliskileri |
| `kv_store` | pulsivo-salesman.db | Anahtar-deger deposu |
| `task_queue` | pulsivo-salesman.db | Gorev kuyrugu |
| `usage_events` | pulsivo-salesman.db | LLM kullanim kayitlari |
...
Pulsivo Salesman, su bilesenlerden olusan kapsamli bir Agent Isletim Sistemidir:
...
# Pulsivo Salesman Prospecting Engine — Eksiksiz Sistem Referansi

> Bu dokuman, Pulsivo Salesman B2B prospecting motorunun her detayini kapsar.
...
crates/pulsivo-salesman-api/
  ...
  static/
    ...
    js/
      ...
      api.js                 321 satir   PulsivoSalesmanAPI istemci
...
**Veritabani:** `~/.pulsivo-salesman/sales.db` (SQLite)
...
pub struct SalesEngine {
    db_path: PathBuf,                 // ~/.pulsivo-salesman/sales.db
}
...
```rust
pub fn spawn_sales_scheduler(kernel: Arc<PulsivoSalesmanKernel>) {
...
- **Toast:** PulsivoSalesmanToast.success/error/warn/info/confirm
...
# Pulsivo Salesman Prospecting Engine — Tam Sistem Plani

> Bu dokuman, Pulsivo Salesman'in B2B prospecting ve lead generation motorunu
...
Pulsivo Salesman Prospecting Engine, Rust dilinde yazilmis bir B2B potansiyel musteri
...
- **Dosya:** `crates/pulsivo-salesman-api/src/sales.rs` (~14.466 satir)
- **OAuth:** `crates/pulsivo-salesman-api/src/codex_oauth.rs` (~1.335 satir)
- **UI:** `crates/pulsivo-salesman-api/static/js/pages/sales.js` (~611 satir)
- **Dashboard:** `crates/pulsivo-salesman-api/static/index_body.html` (~508 satir)
- **Veritabani:** `~/.pulsivo-salesman/sales.db` (SQLite, 8 tablo)
...
```
┌──────────────────────────────────────────────────────────┐
│  [Logo] Pulsivo Salesman Prospecting Harness        [☼][◯][☾]   │
│                                           [Baglanti:●]   │
├──────────────────────────────────────────────────────────┤
│                                                           │
...
**Pulsivo Salesman entegrasyonu:**
```
Pulsivo Salesman (Rust) ←→ Graphiti (Python):
...
## 31. Pulsivo Salesman Platformundan Kullanilacak Parcalar

| Pulsivo Salesman Parcasi | Kullanim |
|-----------------|----------|
...
1. **Parca 1:** Pulsivo Salesman platformunun tum alt sistemleri
