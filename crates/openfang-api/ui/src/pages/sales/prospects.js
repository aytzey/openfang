...
export const salesProspectMixins = {
    ...
    prospectActionSuggestion(p) {
      ...
      if (this.isB2C) {
        if (status === 'contact_ready' && hasLinkedin) return 'Social DM hazir';
        if (hasLinkedin) return 'Profile enrich et';
    ...
    prospectNextAction(prospect) {
      ...
      if (this.isB2C) {
        if (prospect.primary_linkedin_url) {
          return isHigh ? 'Social DM personalizasyonu hazirla' : 'Bio ve lokal sinyalleri zenginlestir';
        }
