...
export const salesApprovalMixins = {
    ...
    async saveApprovalEdit(id) {
      ...
      if (!approval) {
        PulsivoSalesmanToast.error('Duzenlenecek approval bulunamadi');
        return;
      }
      try {
        var data = await PulsivoSalesmanAPI.patch(
        ...
        this.approvalEditorId = '';
        PulsivoSalesmanToast.success('Taslak guncellendi');
      } catch (e) {
        PulsivoSalesmanToast.error(e && e.message ? e.message : 'Taslak guncellenemedi');
      }
    },
    ...
    async bulkApproveSelected() {
      ...
      if (!ids.length) {
        PulsivoSalesmanToast.error('Onaylanacak taslak secilmedi');
        return;
      }
      try {
        var data = await PulsivoSalesmanAPI.post('/api/sales/approvals/bulk-approve', { ids: ids });
        ...
        if (approved > 0) {
          PulsivoSalesmanToast.success(String(approved) + ' taslak onaylandi');
        }
        if (failed > 0) {
          PulsivoSalesmanToast.warn(String(failed) + ' taslak gonderilemedi', 8000);
        }
      } catch (e) {
        PulsivoSalesmanToast.error(e && e.message ? e.message : 'Toplu onay basarisiz');
      }
    },
    ...
    async approve(id) {
      try {
        await PulsivoSalesmanAPI.post('/api/sales/approvals/' + encodeURIComponent(id) + '/approve', {});
        PulsivoSalesmanToast.success('Onaylandi');
        await Promise.all([this.loadApprovals(), this.loadDeliveries()]);
      } catch (e) {
        PulsivoSalesmanToast.error(e && e.message ? e.message : 'Onay islemi basarisiz');
      }
    },
    async reject(id) {
      var self = this;
      PulsivoSalesmanToast.confirm('Taslagi Reddet', 'Bu mesaj taslagi reddedilsin mi?', async function() {
        try {
          await PulsivoSalesmanAPI.post('/api/sales/approvals/' + encodeURIComponent(id) + '/reject', { reason: 'manual_reject' });
          PulsivoSalesmanToast.success('Reddedildi');
          await self.loadApprovals();
        } catch (e) {
          PulsivoSalesmanToast.error(e && e.message ? e.message : 'Reddetme basarisiz');
        }
      });
    },
};
