...
export const salesJobMixins = {
    ...
    startJobPolling(jobId) {
      ...
      var tick = async function() {
        try {
          var data = await PulsivoSalesmanAPI.get('/api/sales/jobs/' + encodeURIComponent(jobId) + '/progress');
          ...
          if (!data || data.status === 'completed' || data.status === 'failed') {
            ...
            if (data && data.status === 'completed') {
              PulsivoSalesmanToast.success(self.isB2C ? 'Social discovery job tamamlandi' : 'Prospecting job tamamlandi');
            } else if (data && data.error_message) {
              PulsivoSalesmanToast.error(data.error_message);
            } else {
              PulsivoSalesmanToast.error(self.isB2C ? 'Social discovery job basarisiz' : 'Prospecting job basarisiz');
            }
            return;
          }
        } catch (e) {
          self.stopJobPolling();
          PulsivoSalesmanToast.error(e && e.message ? e.message : 'Job ilerlemesi alinamadi');
          return;
        }
    ...
    async retryJob(forceFresh) {
      if (!this.currentJobId) {
        PulsivoSalesmanToast.error('Tekrar denenecek job bulunamadi');
        return;
      }
