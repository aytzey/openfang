...
export const salesDataMixins = {
    async init() {
      ...
      window.addEventListener('message', function(evt) {
        ...
        if (data && data.type === 'pulsivo-salesman:codex_oauth' && data.status === 'connected') {
    ...
    async restoreJobProgress() {
      ...
      try {
        var active = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/jobs/active'));
      ...
      try {
        var progress = await PulsivoSalesmanAPI.get('/api/sales/jobs/' + encodeURIComponent(storedJobId) + '/progress');
    ...
    async loadOnboardingStatus() {
      var data = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/onboarding/status'));
    ...
    async loadProfile() {
      var data = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/profile'));
    ...
    async saveProfile() {
      ...
      try {
        ...
        var data = await PulsivoSalesmanAPI.put(this.buildSalesUrl('/api/sales/profile'), payload);
        if (data && data.profile) {
          this.profile = this.normalizeProfile(data.profile);
        }
        PulsivoSalesmanToast.success('Profil kaydedildi');
        await this.loadOnboardingStatus();
      } catch (e) {
        PulsivoSalesmanToast.error(e && e.message ? e.message : 'Profil kaydedilemedi');
      }
    ...
    async saveBriefAndAutofill() {
      if (!this.profileBrief || this.profileBrief.trim().length < 20) {
        PulsivoSalesmanToast.error('Brief cok kisa. En az 20 karakter girin.');
        return;
      }
      this.autofillingProfile = true;
      try {
        var data = await PulsivoSalesmanAPI.post(this.buildSalesUrl('/api/sales/onboarding/brief'), {
        ...
        PulsivoSalesmanToast.success('Brief cozuldu ve profil dolduruldu (' + source + ')');
        if (data && Array.isArray(data.warnings) && data.warnings.length) {
          PulsivoSalesmanToast.warn(data.warnings[0], 8000);
        }
      } catch (e) {
        PulsivoSalesmanToast.error(e && e.message ? e.message : 'Brief islenemedi');
      }
    ...
    async runNow() {
      ...
      try {
        var data = await PulsivoSalesmanAPI.post(this.buildSalesUrl('/api/sales/run'), {});
    ...
    async loadRuns() {
      var data = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/runs', { limit: 20 }));
      this.runs = data.runs || [];
    },

    async loadProspects() {
      var data = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/prospects', { limit: 200 }));
    ...
    async loadLeads() {
      ...
      var data = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/leads', { limit: 200 }));
    ...
    async loadRunProspects(runId) {
      ...
      var data = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/prospects', { limit: 100, run_id: runId }));
    ...
    async loadRunLeads(runId) {
      ...
      var data = await PulsivoSalesmanAPI.get(this.buildSalesUrl('/api/sales/leads', { limit: 100, run_id: runId }));
    ...
    async loadSourceHealth() {
      var data = await PulsivoSalesmanAPI.get('/api/sales/source-health');
    ...
    async loadSelectedDossier() {
      ...
      try {
        var data = await PulsivoSalesmanAPI.get('/api/sales/accounts/' + encodeURIComponent(prospect.company_domain) + '/dossier');
