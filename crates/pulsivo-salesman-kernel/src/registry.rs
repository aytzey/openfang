...
use dashmap::DashMap;
use pulsivo_salesman_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> PulsivoSalesmanResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(PulsivoSalesmanError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> PulsivoSalesmanResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| PulsivoSalesmanError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> PulsivoSalesmanResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| PulsivoSalesmanError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> PulsivoSalesmanResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| PulsivoSalesmanError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: pulsivo_salesman_types::agent::SessionId,
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: pulsivo_salesman_types::agent::AgentIdentity,
    ) -> PulsivoSalesmanResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> PulsivoSalesmanResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| PulsivoSalesmanError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> PulsivoSalesmanResult<()> {
        if self.name_index.contains_key(&new_name) {
            return Err(PulsivoSalesmanError::AgentAlreadyExists(new_name));
        }
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> PulsivoSalesmanResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| PulsivoSalesmanError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> PulsivoSalesmanResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| PulsivoSalesmanError::AgentNotFound(id.to_string()))?;
...
mod tests {
    use super::*;
    use chrono::Utc;
    use pulsivo_salesman_types::agent::*;
