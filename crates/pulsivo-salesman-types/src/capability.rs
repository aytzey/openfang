...
//!
//! PulsivoSalesman uses capability-based security: an agent can only perform actions
...
pub enum Capability {
    ...
    // -- OFP (PulsivoSalesman Wire Protocol) --
...
impl CapabilityCheck {
    ...
    pub fn require(&self) -> Result<(), crate::error::PulsivoSalesmanError> {
        match self {
            Self::Granted => Ok(()),
            Self::Denied(reason) => Err(crate::error::PulsivoSalesmanError::CapabilityDenied(
                reason.clone(),
            )),
        }
    }
