...
use chrono::Utc;
use pulsivo_salesman_types::agent::AgentId;
...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> PulsivoSalesmanResult<()> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn query_hourly(&self, agent_id: AgentId) -> PulsivoSalesmanResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_daily(&self, agent_id: AgentId) -> PulsivoSalesmanResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_monthly(&self, agent_id: AgentId) -> PulsivoSalesmanResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_global_hourly(&self) -> PulsivoSalesmanResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_global_monthly(&self) -> PulsivoSalesmanResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> PulsivoSalesmanResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> PulsivoSalesmanResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
        ...
        for row in rows {
            results.push(row.map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?);
        }
        Ok(results)
    }

    /// Query daily usage breakdown for the last N days.
    pub fn query_daily_breakdown(&self, days: u32) -> PulsivoSalesmanResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_first_event_date(&self) -> PulsivoSalesmanResult<Option<String>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn query_today_cost(&self) -> PulsivoSalesmanResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn cleanup_old(&self, days: u32) -> PulsivoSalesmanResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
