...
use pulsivo_salesman_types::agent::{AgentEntry, AgentId, SessionId};
use pulsivo_salesman_types::error::{PulsivoSalesmanError, PulsivoSalesmanResult};
use pulsivo_salesman_types::memory::{
...
impl MemorySubstrate {
    ...
    pub fn save_agent(&self, entry: &AgentEntry) -> PulsivoSalesmanResult<()> {
        self.structured.save_agent(entry)
    }

    /// Load an agent entry from persistent storage.
    pub fn load_agent(&self, agent_id: AgentId) -> PulsivoSalesmanResult<Option<AgentEntry>> {
        self.structured.load_agent(agent_id)
    }

    /// Remove an agent from persistent storage and cascade-delete sessions.
    pub fn remove_agent(&self, agent_id: AgentId) -> PulsivoSalesmanResult<()> {
    ...
    pub fn load_all_agents(&self) -> PulsivoSalesmanResult<Vec<AgentEntry>> {
        self.structured.load_all_agents()
    }

    /// List all saved agents.
    pub fn list_agents(&self) -> PulsivoSalesmanResult<Vec<(String, String, String)>> {
    ...
    pub fn structured_delete(&self, agent_id: AgentId, key: &str) -> PulsivoSalesmanResult<()> {
    ...
    /// Get a session by ID.
    pub fn get_session(&self, session_id: SessionId) -> PulsivoSalesmanResult<Option<Session>> {
        self.sessions.get_session(session_id)
    }

    /// Save a session.
    pub fn save_session(&self, session: &Session) -> PulsivoSalesmanResult<()> {
        self.sessions.save_session(session)
    }

    /// Create a new empty session for an agent.
    pub fn create_session(&self, agent_id: AgentId) -> PulsivoSalesmanResult<Session> {
        self.sessions.create_session(agent_id)
    }

    /// List all sessions with metadata.
    pub fn list_sessions(&self) -> PulsivoSalesmanResult<Vec<serde_json::Value>> {
        self.sessions.list_sessions()
    }

    /// Delete a session by ID.
    pub fn delete_session(&self, session_id: SessionId) -> PulsivoSalesmanResult<()> {
    ...
    pub fn find_session_by_label(
        ...
        agent_id: AgentId,
        label: &str,
    ) -> PulsivoSalesmanResult<Option<Session>> {
    ...
    pub fn store_llm_summary(
        ...
        agent_id: AgentId,
        summary: &str,
        kept_messages: Vec<pulsivo_salesman_types::message::Message>,
    ) -> PulsivoSalesmanResult<()> {
        self.sessions
    ...
    pub fn append_canonical(
        &self,
        agent_id: AgentId,
        messages: &[pulsivo_salesman_types::message::Message],
    ...
    pub fn load_paired_devices(&self) -> PulsivoSalesmanResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?;
        ...
        for row in rows {
            devices.push(row.map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?);
        }
        Ok(devices)
    }
    ...
    pub fn remove_paired_device(&self, device_id: &str) -> PulsivoSalesmanResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?;
    ...
    ) -> PulsivoSalesmanResult<MemoryId> {
        self.semantic
    ...
    ) -> PulsivoSalesmanResult<Vec<MemoryFragment>> {
        self.semantic
    ...
    pub fn recall_scored_with_embedding(
        ...
        filter: Option<MemoryFilter>,
        query_embedding: Option<&[f32]>,
    ) -> PulsivoSalesmanResult<Vec<ScoredMemoryMatch>> {
        self.semantic
            .recall_scored_with_embedding(query, limit, filter, query_embedding)
    }

    /// Update the embedding for an existing memory.
    pub fn update_embedding(&self, id: MemoryId, embedding: &[f32]) -> PulsivoSalesmanResult<()> {
    ...
    pub async fn recall_with_embedding_async(
        ...
        .await
        .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?
    }
    ...
    pub async fn task_post(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let db = conn.lock().map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
            db.execute(
    ...
    pub async fn task_claim(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match result {
                ...
                Err(e) => Err(PulsivoSalesmanError::Memory(e.to_string())),
            }
        })
        .await
        .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?
    ...
    pub async fn task_complete(&self, task_id: &str, result: &str) -> PulsivoSalesmanResult<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if rows == 0 {
                return Err(PulsivoSalesmanError::Internal(format!("Task not found: {task_id}")));
            }
            Ok(())
        })
        .await
    ...
    pub async fn task_list(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = db.prepare(sql).map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?;
            ...
            for row in rows {
                tasks.push(row.map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?);
            }
            Ok(tasks)
        })
        .await
...
impl Memory for MemorySubstrate {
    ...
    async fn delete(&self, agent_id: AgentId, key: &str) -> PulsivoSalesmanResult<()> {
        let store = self.structured.clone();
        let key = key.to_string();
    ...
    async fn forget(&self, id: MemoryId) -> PulsivoSalesmanResult<()> {
        let store = self.semantic.clone();
    ...
    async fn add_entity(&self, entity: Entity) -> PulsivoSalesmanResult<String> {
    ...
    async fn add_relation(&self, relation: Relation) -> PulsivoSalesmanResult<String> {
        let store = self.knowledge.clone();
    ...
    async fn query_graph(&self, pattern: GraphPattern) -> PulsivoSalesmanResult<Vec<GraphMatch>> {
        let store = self.knowledge.clone();
    ...
    async fn consolidate(&self) -> PulsivoSalesmanResult<ConsolidationReport> {
    ...
    async fn export(&self, format: ExportFormat) -> PulsivoSalesmanResult<Vec<u8>> {
        let _ = format;
        Ok(Vec::new())
    }
