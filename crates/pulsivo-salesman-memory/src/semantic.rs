...
use pulsivo_salesman_types::memory::{MemoryFilter, MemoryFragment, MemoryId, MemorySource};
...
impl SemanticStore {
    ...
    pub fn forget(&self, id: MemoryId) -> PulsivoSalesmanResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
...
fn lookup_candidates(
    ...
    limit: usize,
    filter: Option<&MemoryFilter>,
) -> PulsivoSalesmanResult<Vec<LookupCandidate>> {
    ...
    for row in rows {
        candidates.push(row.map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?);
    }
...
fn recent_candidates(
...
) -> PulsivoSalesmanResult<Vec<String>> {
    if limit == 0 {
        return Ok(Vec::new());
    }
    ...
    let mut ids = Vec::new();
    for row in rows {
        ids.push(row.map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?);
    }
    Ok(ids)
}
...
fn fetch_fragments_by_ids(
    ...
    for row in rows {
        fragments.push(row.map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?);
    }
    Ok(fragments)
}
