...
use chrono::Utc;
use pulsivo_salesman_types::agent::{AgentId, SessionId};
use pulsivo_salesman_types::error::{PulsivoSalesmanError, PulsivoSalesmanResult};
use pulsivo_salesman_types::message::{ContentBlock, Message, MessageContent, Role};
...
impl SessionStore {
        ...
        match result {
            Ok((agent_str, messages_blob, tokens, label)) => {
                ...
                let messages: Vec<Message> = rmp_serde::from_slice(&messages_blob)
                    .map_err(|e| PulsivoSalesmanError::Serialization(e.to_string()))?;
                Ok(Some(Session {
                    id: session_id,
                    agent_id,
                    messages,
    ...
    pub fn delete_agent_sessions(&self, agent_id: AgentId) -> PulsivoSalesmanResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    pub fn list_sessions(&self) -> PulsivoSalesmanResult<Vec<serde_json::Value>> {
        ...
        for row in rows {
            sessions.push(row.map_err(|e| PulsivoSalesmanError::Memory(e.to_string()))?);
        }
        Ok(sessions)
    }
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> PulsivoSalesmanResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
    ...
    ) -> PulsivoSalesmanResult<CanonicalSession> {
        let mut canonical = self.load_canonical(agent_id)?;
            ...
            if to_compact > canonical.compaction_cursor {
                ...
                for msg in compacting {
                    let role = match msg.role {
                        pulsivo_salesman_types::message::Role::User => "User",
                        pulsivo_salesman_types::message::Role::Assistant => "Assistant",
                        pulsivo_salesman_types::message::Role::System => "System",
                    };
    ...
    ) -> PulsivoSalesmanResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> PulsivoSalesmanResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PulsivoSalesmanError::Internal(e.to_string()))?;
